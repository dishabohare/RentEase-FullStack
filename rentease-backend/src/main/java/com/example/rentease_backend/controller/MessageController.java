...
public class MessageController {
    ...
    public ResponseEntity<?> getConversations(Principal principal) {
        ...
        List<Message> allMessages = messageRepository.findBySenderIdOrReceiverIdOrderByTimestampDesc(user.getId(), user.getId());
        List<ConversationSummary> summaries = new ArrayList<>();
        Set<Long> processedUsers = new HashSet<>();

        for (Message msg : allMessages) {
            User otherUser = msg.getSender().getId().equals(user.getId()) ? msg.getReceiver() : msg.getSender();
            if (!processedUsers.contains(otherUser.getId())) {
                processedUsers.add(otherUser.getId());
                long unreadCount = messageRepository.countByReceiverIdAndSenderIdAndIsReadFalse(user.getId(), otherUser.getId());
    ...
    public ResponseEntity<?> getMessagesWithUser(@PathVariable Long otherUserId, Principal principal) {
        User user = getAuthenticatedUser(principal);
        if (user == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("message", "User not authenticated"));
        }
        ...
        List<Message> chatHistory = messageRepository.findChatHistory(user.getId(), otherUserId);
        return ResponseEntity.ok(chatHistory);
    }

    // 3. POST /api/messages
    @PostMapping
    public ResponseEntity<?> sendMessage(@RequestBody SendMessageRequest req, Principal principal) {
        User sender = getAuthenticatedUser(principal);
        if (sender == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("message", "User not authenticated"));
        }

        User receiver = userRepository.findById(req.receiverId()).orElse(null);
        if (receiver == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", "Receiver not found"));
        }
        ...
        notification.setDescription(sender.getName() + " sent you a message: " + 
                (req.content().length() > 50 ? req.content().substring(0, 47) + "..." : req.content()));
    ...
    public ResponseEntity<?> markMessagesRead(@PathVariable Long otherUserId, Principal principal) {
        User user = getAuthenticatedUser(principal);
        if (user == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("message", "User not authenticated"));
        }
